...
//			print_OB_result();	// print result
  ...
  diff = 1000000000 * (end.tv_sec - beg.tv_sec) + end.tv_nsec - beg.tv_nsec;
  printf("diff %10.3f us\n", diff/1000.0);
...
void lookup(void){
  ...
  float index;
  float32x4_t n_index, n_index_2, n_index_3, t_voc;
  uint32x4_t mask_i = vmovq_n_u32(0);
  uint32x4_t change_or_not;
  float32x4_t index_add = {1.0f, 2.0f, 3.0f, 4.0f};
  float32x4_t estVoc_n = vld1q_dup_f32(&estVoc);
 
  for(int a = 0; a < 100; a = a + 4){
    index = (float) a;
    n_index = vdupq_n_f32(index);
    n_index = vaddq_f32(n_index,index_add);
    
    t_voc = vld1q_f32(&table_Voc[a]);
    mask_i = vcgtq_f32(estVoc_n, t_voc);
    n_index_3 = n_index_2;
    n_index_2 = vbslq_f32(mask_i, n_index, n_index_2);
    n_index_2 = vmaxq_f32(n_index_2, n_index_3);
  }
  
  float32x2_t maxOfHalfs = vpmax_f32(vget_low_f32(n_index_2), vget_high_f32(n_index_2));
  float32x2_t maxOfMaxOfHalfs = vpmax_f32(maxOfHalfs, maxOfHalfs);
  float maxValue = vget_lane_f32(maxOfMaxOfHalfs, 0);
  
  int i = (int) maxValue;

  float32x4_t table_SOC_n0 = vld1q_dup_f32(&table_SOC[i-1]);
  float32x4_t table_SOC_n1 = vld1q_dup_f32(&table_SOC[i]);
  float32x4_t table_Voc_n0 = vld1q_dup_f32(&table_Voc[i-1]);
  float32x4_t table_Voc_n1 = vld1q_dup_f32(&table_Voc[i]);
  
  float32x4_t t1 = vsubq_f32(table_SOC_n1, table_SOC_n0);
  float32x4_t t2 = vsubq_f32(table_Voc_n1, table_Voc_n0);
  float32x4_t t3 = vrecpeq_f32(t2);
  float32x4_t slope_n = vmulq_f32(t1, t3);
  
  float32x4_t t4 = vsubq_f32(estVoc_n, table_Voc_n0);
  float32x4_t t5 = vmulq_f32(slope_n, t4);
  float32x4_t estSOC_n = vaddq_f32(table_SOC_n0, t5);
  
  //saturation
  float32x4_t t_Voc99 = vld1q_dup_f32(&table_Voc[99]);
  float32x4_t t_Voc0 = vld1q_dup_f32(&table_Voc[0]);
  float32x4_t t_SOC99 = vld1q_dup_f32(&table_SOC[99]);
  float32x4_t t_SOC0 = vld1q_dup_f32(&table_SOC[0]);
  
  uint32x4_t mask = vcgeq_f32(estVoc_n, t_Voc99);
  estSOC_n = vbslq_f32(mask, t_SOC99, estSOC_n);
  
  uint32x4_t mask2 = vcleq_f32(estVoc_n, t_Voc0);
  estSOC_n = vbslq_f32(mask2, t_SOC0, estSOC_n);

  slope = vgetq_lane_f32(slope_n, 0);
  estSOC = vgetq_lane_f32(estSOC_n, 0);
...
void estimateSOC(int i1){
	...
	Ad[0][1] = 0;
	Ad[1][0] = 0;
	Ad[1][1] = expf(-time_OB / R1 / C1);	// e^(-Ts/RC)
	...
	pole_d[0] = expf(pole_c[0] * time_OB);
	pole_d[1] = expf(pole_c[1] * time_OB);
  ...
  float temp5[2] = {N_matI[0][0], N_matI[1][0]};
  float temp6[2] = {N_matI[0][1], N_matI[1][1]};
  
  float32x2_t nmati_n0 = vld1_f32(temp5);
  float32x2_t nmati_n1 = vld1_f32(temp6);

  // Step 7: Calculate Ad^2 =  A * A
  float temp[4] = {Ad[0][0], Ad[1][0], Ad[0][1], Ad[1][1]};
  float32x2_t Ad_n0 = vld1_f32(&temp[0]);
  float32x2_t Ad_n1 = vld1_f32(&temp[2]);

  float32x2_t AA_n0 = vadd_f32(vmul_n_f32(Ad_n0, Ad_n0[0]), vmul_n_f32(Ad_n1, Ad_n0[1]));
  float32x2_t AA_n1 = vadd_f32(vmul_n_f32(Ad_n0, Ad_n1[0]), vmul_n_f32(Ad_n1, Ad_n1[1]));
  
  // Step 8: alpha1 * Ad = Ad_alpha
  float32x2_t Ad_alp_n0 = vmul_n_f32(Ad_n0, alpha1);
  float32x2_t Ad_alp_n1 = vmul_n_f32(Ad_n1, alpha1);
  
  // Step 9: AA = Ad^2 + alpha1 * Ad + alpha2 * I
  float temp1[2] = {alpha2, 0};
  float temp2[2] = {0, alpha2};
  float32x2_t alpha2_n0 = vld1_f32(temp1);
  float32x2_t alpha2_n1 = vld1_f32(temp2);
  
  AA_n0 = vadd_f32(AA_n0, vadd_f32(Ad_alp_n0, alpha2_n0));
  AA_n1 = vadd_f32(AA_n1, vadd_f32(Ad_alp_n1, alpha2_n1));
  
  // Step 10: L1 = phi(A) * [C CA]^-1  = AA * N_matI
  float32x2_t L1_n0 = vadd_f32(vmul_n_f32(AA_n0, nmati_n0[0]), vmul_n_f32(AA_n1, nmati_n0[1]));
  float32x2_t L1_n1 = vadd_f32(vmul_n_f32(AA_n0, nmati_n1[0]), vmul_n_f32(AA_n1, nmati_n1[1]));
  
  // Step 11: observer gain L_d =  phi(Ad)*N_matI*[0 0 ... 1]'
  L_d[0] = vget_lane_f32(L1_n1, 0);
  L_d[1] = vget_lane_f32(L1_n1, 1);
